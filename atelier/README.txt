...
atelier ships five dependency-free Python helpers in `scripts/`, for the few tasks that are exact computation. The agent runs them; you never touch a terminal.

| Script | Purpose |
|---|---|
| `validate_skill.py` | Verifies the skill is sound before commit: routing resolves, frontmatter is valid, and the Karpathy guidelines are still byte-for-byte verbatim (checksum). |
| `spacing_scale.py` | Generates a hierarchical spacing scale from the ~40% proximity rule (8px snapping). |
| `type_scale.py` | Generates a modular type scale with line-heights, rem values, and a fluid `clamp()`, per the typography rules. |
| `contrast_check.py` | Computes exact WCAG contrast ratios and pass/fail for a color pair. |
| `mobile_check.py` | Scans HTML/CSS/JSX for mobile-hostile patterns: disabled zoom, fixed widths, tiny input fonts, sub-44px touch targets, missing viewport. |
...
- **apple-easings** — authored by Anthony Perrault, derived from his personal After Effects motion library; physics translation grounded in WWDC documentation and SwiftUI spring research; preserved in full and verbatim
